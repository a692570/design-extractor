...
const SECTION_ORDER = [
  ...
  { key: 'sections', label: 'Page Section Styles' },
...
function buildSectionClassMap(components) {
  ...
  for (const [key, value] of Object.entries(components)) {
    if (typeof value === 'string' && value) {
      map[key] = extractClassNamesFromHtml(value);
    } else if (Array.isArray(value)) {
      // sections[] — merge class names from every section into one bucket
      const merged = new Set();
      for (const item of value) {
        if (item && typeof item.html === 'string') {
          for (const c of extractClassNamesFromHtml(item.html)) merged.add(c);
        }
      }
      if (merged.size) map[key] = merged;
    }
  }
  return map;
}
